...
dimension = 1000
grid = [[False for _ in range(dimension)] for _ in range(dimension)]
...
Instruction = namedtuple('Instruction', 'start_x, start_y, end_x, end_y, action')


def parse(instruction):
    for action in actions:
        if instruction.startswith(action):
            r1, _, r2 = instruction[len(action):].partition('through')
            start_x, start_y = map(int, r1.strip().split(','))
            end_x, end_y = map(int, r2.strip().split(','))
            return Instruction(start_x, start_y, end_x, end_y, action)


def perform(instruction_str):
    ins = parse(instruction_str)

    for x in range(ins.start_x, ins.end_x + 1):
        for y in range(ins.start_y, ins.end_y + 1):
            if ins.action == TURN_ON:
                grid[y][x] = True
            elif ins.action == TURN_OFF:
                grid[y][x] = False
            else:
...
num_lights_on = sum(sum(filter(None, row)) for row in grid)
print(num_lights_on)
