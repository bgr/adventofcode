...
Instruction = namedtuple('Instruction', 'start_x, start_y, end_x, end_y, offset')


def parse(instruction):
    for action, offset in brightness_map.items():
        if instruction.startswith(action):
            ...
            return Instruction(start_x, start_y, end_x, end_y, offset)
...
def perform(instruction_str):
    ...
    for x in range(ins.start_x, ins.end_x + 1):
        for y in range(ins.start_y, ins.end_y + 1):
            cur_brightness = grid[y][x]
            grid[y][x] = max(0, cur_brightness + ins.offset)
