...
repeating_regex_part = [r"\w+ (-?\d+)"] * 5
matcher = re.compile(r"\w+: " + ", ".join(repeating_regex_part))

def parse(line):
    c, d, f, t, cal = matcher.match(line).groups()
    return tuple(map(int, (c, d, f, t, cal)))


def calc(ingredients, max_scoop=100, acc_score=(0, 0, 0, 0, 0)):
    if not ingredients:
        return [acc_score]
...
only500cal = [(c, f, d, t) for (c, f, d, t, cal) in combinations if cal == 500]
capped = [tuple(map(lambda n: max(0, n), tup)) for tup in only500cal]
