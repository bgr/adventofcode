...
def parse(line):
    name, _, facts = line.partition(": ")
    fact_parts = [f.partition(": ") for f in facts.split(", ")]
    facts = {name: int(amount) for name, _, amount in fact_parts}
    return (name, facts)

known_facts_lines = [l for l in inp_known_facts.splitlines() if l]
known_facts = {n: int(f) for n, _, f in
               [l.partition(": ") for l in known_facts_lines]}

lines = [l for l in inp.splitlines() if l]
sues = {k: v for k, v in [parse(line) for line in lines]}

for name, facts in sues.items():
    common_keys = set(known_facts.keys()) & set(facts.keys())
    if all(facts[k] == known_facts[k] for k in common_keys):
        print(name)
...
for name, facts in sues.items():
    common_keys = set(known_facts.keys()) & set(facts.keys())
    if all(checkers.get(k, eq)(known_facts[k], facts[k]) for k in common_keys):
        print(name)
